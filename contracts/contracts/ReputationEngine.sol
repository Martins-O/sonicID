...
contract ReputationEngine is Ownable, Pausable {
    ...
    modifier onlyPlatformOperator() {
        require(
            platformOperators[msg.sender] || msg.sender == owner(),
            "Not authorized platform operator"
        );
        _;
    }
    ...
    function updateReputation(
        ...
        uint256 newPlatformScore = _calculateNewScore(
            currentPlatformScore,
            _scoreChange
        );
        ...
        uint256 newCategoryScore = _calculateNewScore(
    ...
    function _calculateNewScore(
        ...
        if (_change >= 0) {
            ...
            return
                _currentScore + increase > MAX_PLATFORM_SCORE
                    ? MAX_PLATFORM_SCORE
    ...
    function _trackActivePlatform(
        ...
        for (uint i = 0; i < userRep.activePlatforms.length; i++) {
            if (
                keccak256(bytes(userRep.activePlatforms[i])) ==
    ...
    function _trackActiveCategory(
        ...
        for (uint i = 0; i < userRep.activeCategories.length; i++) {
            if (
                keccak256(bytes(userRep.activeCategories[i])) ==
    ...
    function _applyTimeDecay(address _user) internal {
        ...
        uint256 daysSinceActivity = (block.timestamp - userRep.lastActivity) /
            1 days;
        if (daysSinceActivity >= INACTIVITY_DECAY_DAYS) {
            ...
            userRep.totalScore = userRep.totalScore > decayAmount
                ? userRep.totalScore - decayAmount
                : 0;
        }
    }
